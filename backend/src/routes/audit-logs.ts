...
router.get('/actions', authenticate, requireRole('ADMIN', 'DIRETOR'), async (_req: ExpressRequest, res: Response) => {
  try {
 ...
 */
router.get('/export', authenticate, requireRole('ADMIN', 'DIRETOR'), async (req: ExpressRequest, res: Response) => {
  try {
    const logs = await prisma.auditLog.findMany({
      where: buildWhere(req.query),
      include: { request: { select: { title: true } } },
