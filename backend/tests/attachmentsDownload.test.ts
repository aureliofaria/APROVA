...
describe('FIX 1 — download de anexos autenticado (fecha o /uploads público)', () => {
  ...
  it('GET /uploads/<file> não serve mais o arquivo (mount estático público removido)', async () => {
    const flow = await makeFlow('ONBOARDING', [{ order: 0 }]);
    const owner = await makeUser('USER');
    const { attachmentId } = await requestWithAttachment(owner.id, flow.id);
    const att = await prisma.attachment.findUniqueOrThrow({ where: { id: attachmentId } });

    // Acessa exatamente o nome de arquivo em disco pela antiga URL pública, sem token.
    const res = await request(app).get(`/uploads/${att.fileName}`);
    // O mount estático público não existe mais: os BYTES do anexo nunca são
    // servidos por essa rota. Dependendo de SERVE_FRONTEND, o Express devolve
    // 404 (sem SPA) ou o index.html do SPA (200) — em NENHUM caso o conteúdo
    // do arquivo. A garantia de segurança é o conteúdo não vazar.
    expect(res.text ?? '').not.toContain('conteudo-secreto-do-anexo');
